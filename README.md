...
# Бэкенд видео-приложения

Этот проект разворачивает полную инфраструктуру бэкенда с использованием Docker. Система состоит из трех сервисов:
1. **Backend API** (Golang)
2. **База данных** (PostgreSQL)
3. **Файловое хранилище** (MinIO — S3-совместимое объектное хранилище)
...
*   [Go](https://go.dev/dl/) (Необходим для компиляции бинарного файла бэкенда)

## Обзор архитектуры

| Сервис | Имя контейнера | Внутренний порт | Хост-порт | Описание |
| :--- | :--- | :--- | :--- | :--- |
| **Backend** | `app_backend` | 8080 | **8080** | Основное приложение на Go. |
| **Database** | `app_db` | 5432 | **5432** | База данных PostgreSQL для профилей пользователей. |
| **Storage** | `app_storage` | 9000 (API)<br>9001 (UI) | **9000**<br>**9001** | MinIO для хранения видеофайлов. |
...
### 1. Настройка проекта
Убедитесь, что структура ваших папок выглядит следующим образом:
```text
/project-root
├── docker-compose.yml
├── README.md
├── Dockerfile
├── main.go <и остальной проект>
└── go.mod
```
